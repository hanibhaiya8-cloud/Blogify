...
interface AdminHighProfileFormProps {
  profileId?: string;
}

export function AdminHighProfileForm({ profileId }: AdminHighProfileFormProps) {
  ...
  useEffect(() => {
    const loadProfile = async () => {
      if (isEditMode && profileId) {
        try {
          setIsLoading(true);
          const data = await highProfileCallGirlsApi.getById(profileId);
          setProfile({
          ...
          console.error('Error loading high profile call girl:', error);
          // Fallback to localStorage if API fails
          if (typeof window !== 'undefined') {
            const savedProfiles = localStorage.getItem('blogify_high_profile_call_girls');
            if (savedProfiles) {
              const parsedProfiles = JSON.parse(savedProfiles);
              const existingProfile = parsedProfiles.find((p: any) => (p.id || p._id) === profileId);
              
              if (existingProfile) {
                setProfile({
                  heading: existingProfile.heading,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditMode && profileId) {
        await highProfileCallGirlsApi.update(profileId, profile);
      } else {
        await highProfileCallGirlsApi.create(profile);
      }

      // Redirect to user page so the new data is visible immediately
      router.push('/user');
    } catch (err: any) {
      console.error('Error saving high profile call girl:', err);
      // Fallback to localStorage if API fails
      if (typeof window !== 'undefined') {
        const savedProfiles = localStorage.getItem('blogify_high_profile_call_girls');
        ...
        localStorage.setItem('blogify_high_profile_call_girls', JSON.stringify(profiles));
        router.push('/user');
      }
      ...
      <div className="bg-white rounded-2xl shadow-xl p-4 sm:p-6">
        <div className="flex justify-between items-center mb-4 sm:mb-6">
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900">
            {isEditMode ? 'Edit High Profile Call Girl' : 'Add New High Profile Call Girl'}
          </h2>
          <button
            onClick={() => router.push('/')}
            ...
            <div className="space-y-3">
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-xl p-3 sm:p-4 text-center">
                ...
                <input
                  ...
                  className="hidden"
                  id="high-profile-image-upload"
                />
                <label
                  htmlFor="high-profile-image-upload"
          ...
          <div className="flex flex-col-reverse sm:flex-row justify-end gap-3 sm:gap-4 pt-4">
            ...
            >
              {isLoading ? (
                ...
                'Update High Profile Girl'
              ) : (
                'Add High Profile Girl'
              )}
            </button>
          </div>
        </form>
